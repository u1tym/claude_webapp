...
from app.routers.knowhows import router as knowhows_router
from app.routers.major_categories import router as major_categories_router
from app.routers.middle_categories import router as middle_categories_router
...
def create_app() -> FastAPI:
    cfg = load_config()
    setup_logging()
    app = FastAPI(title="knowhow-management", redirect_slashes=False)
    app.add_middleware(
        CORSMiddleware,
    ...
    app.include_router(major_categories_router)
    app.include_router(middle_categories_router)
    app.include_router(knowhows_router)
