...
def load_config() -> Config:
    ...
    ping_timeout_raw = values.get("PING_TIMEOUT_SECONDS") or "2"
    return Config(
        ...
        ping_host=(values.get("PING_HOST") or "").strip(),
        ping_timeout_seconds=int(ping_timeout_raw),
        wol_mac=(values.get("WOL_MAC") or "").strip(),
    )
