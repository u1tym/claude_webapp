...
<template>
  ...
  <div v-else-if="settings" class="shell">
    ...
    <nav class="nav" aria-label="ナビ">
      <div class="nav-brand pc-only">グッズ管理</div>
      <button class="nav-item is-current" type="button">グッズ管理</button>
    </nav>
    <main class="content">
      <RouterView />
    </main>
