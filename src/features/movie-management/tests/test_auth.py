...
def test_expired_session_is_401(make_user: Callable[..., TestUser]) -> None:
    assert make_user(expired=True).client.get("/genres").status_code == 401


def test_deleted_user_is_401(make_user: Callable[..., TestUser]) -> None:
    assert make_user(deleted=True).client.get("/genres").status_code == 401


def test_unassigned_user_is_403(make_user: Callable[..., TestUser]) -> None:
    res = make_user(assigned=False).client.get("/genres")
...
def test_deleted_feature_is_403(user: TestUser) -> None:
    with get_conn() as conn, conn.cursor() as cur:
        cur.execute("UPDATE public.features SET is_deleted = true WHERE id = 'movie-management'")
    try:
        assert user.client.get("/genres").status_code == 403
    finally:
        with get_conn() as conn, conn.cursor() as cur:
            cur.execute("UPDATE public.features SET is_deleted = false WHERE id = 'movie-management'")
...
def test_session_expiry_is_extended(user: TestUser) -> None:
    ...
    assert user.client.get("/genres").status_code == 200
    with get_conn() as conn, conn.cursor() as cur:
        cur.execute(
