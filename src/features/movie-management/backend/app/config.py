...
# 既定は backend/.env。MOVIE_MANAGEMENT_ENV_FILE で別の設定ファイルを指定できる
# （例: 移行プログラムの移行先を、本番の DB にする）。
ENV_PATH = Path(os.environ.get("MOVIE_MANAGEMENT_ENV_FILE") or BACKEND_DIR / ".env")

FEATURE_ID = "movie-management"
