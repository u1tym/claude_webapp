...
# 既定は backend/.env。NOTE_MANAGEMENT_ENV_FILE で別の設定ファイルを指定できる
# （例: 移行プログラムの移行先を、本番の DB にする）。
ENV_PATH = Path(os.environ.get("NOTE_MANAGEMENT_ENV_FILE") or BACKEND_DIR / ".env")

FEATURE_ID = "note-management"
...
def load_config() -> Config:
    ...
    return Config(
        ...
        parts_max_revisions=int(values.get("PARTS_MAX_REVISIONS") or "3"),
        part_max_bytes=int(values.get("PART_MAX_BYTES") or "10485760"),
    )
