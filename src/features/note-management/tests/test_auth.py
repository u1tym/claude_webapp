...
def test_expired_session_is_401(make_user: Callable[..., TestUser]) -> None:
    assert make_user(expired=True).client.get("/items").status_code == 401


def test_deleted_user_is_401(make_user: Callable[..., TestUser]) -> None:
    assert make_user(deleted=True).client.get("/items").status_code == 401


def test_unassigned_user_is_403(make_user: Callable[..., TestUser]) -> None:
    res = make_user(assigned=False).client.get("/items")
...
def test_deleted_feature_is_403(user: TestUser) -> None:
    with get_conn() as conn, conn.cursor() as cur:
        cur.execute("UPDATE public.features SET is_deleted = true WHERE id = 'note-management'")
    try:
        assert user.client.get("/items").status_code == 403
    finally:
        with get_conn() as conn, conn.cursor() as cur:
            cur.execute("UPDATE public.features SET is_deleted = false WHERE id = 'note-management'")
...
def test_session_expiry_is_extended(user: TestUser) -> None:
    ...
    assert user.client.get("/items").status_code == 200
    with get_conn() as conn, conn.cursor() as cur:
        cur.execute(
...
def test_debug_user_skips_cookie_but_checks_assignment(
    ...
    assert TestClient(app).get("/items").status_code == 403

    monkeypatch.setattr("app.deps.load_config", lambda: dataclasses.replace(real, debug_user="nm_no_such_user"))
    assert TestClient(app).get("/items").status_code == 401
