...
def test_note_specific_settings_have_defaults(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    cfg = _load(monkeypatch, tmp_path, "DB_SERVER=h\n")
    assert cfg.parts_max_revisions == 3
    assert cfg.part_max_bytes == 10485760


def test_note_specific_settings_are_read(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    cfg = _load(monkeypatch, tmp_path, "PARTS_MAX_REVISIONS=5\nPART_MAX_BYTES=2048\n")
    assert (cfg.parts_max_revisions, cfg.part_max_bytes) == (5, 2048)
