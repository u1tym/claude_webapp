...
予算管理画面（SCR-002）の複製作成で、複製元の予算期間を選べるようにする
...
`POST /budget-periods/{budget_period_id}/duplicate` は既に複製元をパスパラメータで受け取れる（直近固定ではない）ため、API・DB の変更は無い。複製作成フォームに、登録済みの予算期間から選ぶ「複製元の予算期間選択」を追加する（タイトルと開始日〜終了日を添えて表示。既定は直近の予算期間）。`openDuplicatePeriodForm` でこの既定値を設定し、`submitPeriodForm` は `periods.value[0]!.id` の決め打ちをやめて、選択された複製元の `id` を `duplicateBudgetPeriod` に渡す。

### 完了条件

- [ ] 複製作成フォームに複製元の予算期間選択があり、既定で直近の予算期間が選ばれている
- [ ] 直近以外の予算期間を選んで複製作成すると、その予算期間の予算項目が複製される
- [ ] 予算期間が1件も無いときは複製作成ボタンから開けない（従来どおり）
- [ ] ブラウザで一連の操作を確認できる
...
- `design.md` / バックエンド設計・業務ロジック（`app/services/budget_service.py`）
- `ui-design.md` / SCR-002: 予算管理（予算期間一覧・予算項目一覧）
- `api-design.md` / GET `/budget-periods`
...
`GET /budget-periods` の応答に `total_amount`（本人の、その予算期間に属する、削除されていない予算項目の金額合計。0 件なら `"0.00"`）を追加する。`repos.py` で予算期間ごとに `budget_items` を集計するクエリ（または取得後の集計処理）を実装し、`budget_service.py` の一覧取得処理で組み立てる。

`api.ts` の `BudgetPeriod` 型に `total_amount: string` を追加する。`BudgetsView.vue` の一覧表示の予算期間一覧に、各行の金額合計（整数表示）を追加する。詳細表示の予算項目一覧の下に、表示中の予算項目（`items`）の金額合計（フロントエンドで算出、整数表示）を、1件以上あるときだけ表示する。

### 完了条件

- [ ] `GET /budget-periods` の応答の各要素に `total_amount` が含まれ、削除されていない予算項目の金額合計と一致する
- [ ] 予算項目が0件の予算期間は `total_amount` が `"0.00"` になる
- [ ] 予算期間一覧の各行に、その期間の金額合計が整数で表示される
- [ ] 予算項目一覧の下に、表示中の予算項目の金額合計が整数で表示される
- [ ] 予算項目が0件のときは予算項目一覧側の金額合計を表示しない
- [ ] ブラウザで一連の操作を確認できる
