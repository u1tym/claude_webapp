...
機能のアイコンは任意項目である。未設定のときは、応答では空文字列とする。
...
処理概要: ユーザを追加する。パスワードはハッシュして保存する。`email` は任意で、無い／空なら空文字として保存する。メールアドレスはログインに使わない。
...
| `email` を指定していて、`@` が無い、または `@` の前後に文字が無い | 400 |
| 未ログイン | 401 |
| 権限なし | 403 |
...
`password` は省略可。空または省略ならパスワードは変えない。`email` は任意。無い／空なら空文字にする（クリアできる）。
...
`icon` は data URL。アイコンが未設定の機能は空文字列。`is_protected` は識別子が `user-management` なら true。0 件なら空配列。
...
`icon` は data URL。省略可。省略、または空なら、アイコンなしで追加する。
...
`icon` はアイコンなしのときは空文字列。

処理概要: 機能を追加する。アイコンを指定したときは、バイト列として保存する。

エラー:

| 状況 | 応答 |
|------|------|
| `id`・`title`・`url` のいずれかが無い／空、または `icon` を送ったが data URL でない | 400 |
| 未ログイン | 401 |
| 権限なし | 403 |
...
`icon` は省略可。指定すれば、そのアイコンに差し替える。`remove_icon` は既定 `false`。`true` を指定すると、アイコンを削除する（このとき `icon` は無視する）。`icon` を省略し、かつ `remove_icon` が `false`（省略時を含む）なら、既存のアイコン設定を維持する（アイコンが未設定の機能は未設定のまま）。識別子はパスで指定し、本文では変えない。
...
処理概要: 未削除機能のタイトル、遷移先を更新する。アイコンは、指定があれば差し替え、削除が指定されれば外し、どちらもなければ現状を維持する。
...
| REQ-003 | POST `/users`（`email` は任意。指定時の形式不正は 400） |
| REQ-004 | PATCH `/users/{user_id}`（`email` は任意。指定時の形式不正は 400） |
...
| 2026-09-22 13:25 | 未承認 | POST・PATCH `/users` の `email` を必須から任意に変更。無い／空は 400 にせず空文字として扱う。指定時のみ形式検査（REQ-003・004） |
| 2026-09-22 13:26 | 承認済み | `email` 任意化を承認 |
