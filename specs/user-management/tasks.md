...
- `design.md` / バリデーション、バックエンド設計・業務ロジック
- `ui-design.md` / SCR-001 部品詳細（メールアドレス）
- `api-design.md` / POST・PATCH `/users`
- `db-design.md` / `public.users`（記述のみ。列・型は変更なし）
...
`app/routers/users.py` の `email` バリデータ（`email_not_blank`）を、空文字を許容するように変更する。値が空でないときだけ `user_service.email_is_valid` で形式を検査し、不正なら拒否する。空はそのまま通す。

`app/services/user_service.py` の `_require_email` を、空を拒否しないように変更する（関数名も実態に合わせて見直してよい）。空ならそのまま（空文字）を返し、空でなければ形式検査だけ行う。

フロントエンドの登録・更新フォームで、メールアドレス欄を必須項目の見た目・検証から外す（プレースホルダ「メールアドレス」は維持）。保存時、メールアドレスが空でも送信できるようにする。値があるときだけ簡易な形式チェック（`@` の前後に文字がある）を行う。

### 完了条件

- [ ] メールアドレスを空、または未指定でユーザを追加できる
- [ ] メールアドレスを空にしてユーザを更新できる
- [ ] メールアドレスを指定したとき、形式が不正なら追加・更新とも失敗する（400）
- [ ] ユーザ名またはパスワード（追加時）が空のときは、従来どおり失敗する
- [ ] ブラウザで一連の操作を確認できる
...
- [ ] メールアドレスが任意であること（空・未指定で追加・更新できる）、指定時の形式不正、一覧への出力を確認する
...
| 2026-09-22 13:30 | 未承認 | メールアドレスを任意入力にする（タスク17）。API・画面 |
| 2026-09-22 13:31 | 承認済み | タスク17を承認 |
