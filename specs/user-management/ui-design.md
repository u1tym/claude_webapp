...
| SCR-003 | 割当 | `/portal_user_management/assignments` | 割当の一覧、追加、表示順の更新、解除。 | REQ-001, REQ-010, REQ-011, REQ-012, REQ-014 |
...
| メールアドレス | 任意 | 空、または選択行のメールアドレス | 指定されているとき、`@` の前後に文字が無ければ送信しない。空は送信できる | プレースホルダ「メールアドレス」 |
...
| メイン | エラー | 失敗時、領域先頭に一文。内部理由は出さない。 |
| メイン | 成功 | 保存・削除の成功時、領域先頭に一文。数秒で消す。 |
| メイン | 一覧 | 列はアイコン、識別子、タイトル。行を選ぶと編集対象になる。アイコンが未設定の行はアイコンを表示しない。空のときは「データがありません」。 |
| メイン | 主ボタン | 「新規」。追加の入力に切り替える。 |
| メイン | 入力 | 識別子。追加時のみ入力可。更新時は表示のみ。プレースホルダ「識別子」。 |
| メイン | 入力 | タイトル。プレースホルダ「タイトル」。 |
| メイン | 入力 | 遷移先。プレースホルダ「遷移先」。 |
| メイン | 入力 | アイコン。画像ファイルを選ぶ。任意項目。追加・更新とも未選択なら既存（追加時はなし）を維持する。選んだ内容は画像として示す。 |
| メイン | チェックボックス | 「アイコンを削除する」。更新時、編集対象の機能に既にアイコンが設定されているときだけ出す。選ぶとアイコン欄は無視し、保存でアイコンを外す。 |
| メイン | 主ボタン | 「保存」。追加または更新を送る。 |
| メイン | 副ボタン | 「キャンセル」。入力を捨て、選択を解除する。 |
...
| アイコン | 任意 | なし、または選択行の画像 | 未選択なら送信しない（既存を維持、追加時はなしのまま） | 配信用ファイルは置かない |
| チェックボックス「アイコンを削除する」 | - | 未選択 | - | 更新時、既存アイコンがある行を編集しているときだけ表示。選ぶとアイコン欄より優先し、削除で送信 |
| 主ボタン「新規」 | - | - | - | 読込中は無効 |
| 主ボタン「保存」 | - | - | - | 読込中は無効 |
| 副ボタン「キャンセル」 | - | - | - | 読込中は無効 |
...
| 操作 | 部品 | 結果 |
|------|------|------|
| 新規 | 主ボタン「新規」 | 入力を空にし、追加の入力を出す。識別子は入力可。「アイコンを削除する」は出さない。 |
| 行を選ぶ | 一覧の行 | その機能を編集対象にする。識別子は表示のみ。その機能にアイコンが設定されていれば「アイコンを削除する」を出す。 |
| アイコンを削除する | チェックボックス | 選ぶとアイコン欄を無視の対象にする。 |
| 保存 | 主ボタン「保存」 | 必須が空なら送信せず、対象欄を示す。「アイコンを削除する」を選んでいればアイコンを外して保存する。それ以外でアイコンを選んでいればそのアイコンに差し替える。どちらもなければ既存のアイコン設定を維持する。成功なら一覧を更新し、入力を閉じる。失敗ならエラーを示し、入力は残す。 |
| キャンセル | 副ボタン「キャンセル」 | 入力を捨て、選択を解除する。 |
...
- 目的: 未削除ユーザへの未削除機能の割当を一覧し、追加・表示順の更新・解除する。
- 対応 REQ: REQ-001, REQ-010, REQ-011, REQ-012, REQ-014
...
|------|------|------------|
| メイン | エラー | 失敗時、領域先頭に一文。内部理由は出さない。 |
| メイン | 成功 | 割当・更新・解除の成功時、領域先頭に一文。数秒で消す。 |
| メイン | 一覧 | 列はユーザ名、機能タイトル、表示順。行を選ぶと編集対象になる。空のときは「データがありません」。 |
| メイン | 主ボタン | 「新規」。追加の入力に切り替える。 |
| メイン | 入力 | ユーザ。未削除ユーザから選ぶ。編集対象を選んでいるときは変更できない。 |
| メイン | 入力 | 機能。未削除機能から選ぶ。編集対象を選んでいるときは変更できない。 |
| メイン | 入力 | 表示順。プレースホルダ「表示順」。 |
| メイン | 主ボタン | 「保存」。新規なら割当を追加し、編集対象選択中なら表示順を更新する。 |
| メイン | 副ボタン | 「キャンセル」。入力を捨て、選択を解除する。 |
...
| ユーザ | 必須 | 未選択、または選択行のユーザ | 未選択なら送信しない | 未削除ユーザから選ぶ。編集対象を選んでいるときは無効（変更不可） |
| 機能 | 必須 | 未選択、または選択行の機能 | 未選択なら送信しない | 未削除機能から選ぶ。編集対象を選んでいるときは無効（変更不可） |
| 表示順 | 必須 | 空、または選択行の表示順 | 空なら送信しない | プレースホルダ「表示順」。整数 |
| 主ボタン「新規」 | - | - | - | 読込中は無効 |
| 主ボタン「保存」 | - | - | - | 読込中は無効 |
| 副ボタン「キャンセル」 | - | - | - | 読込中は無効 |
...
| 操作 | 部品 | 結果 |
|------|------|------|
| 新規 | 主ボタン「新規」 | 入力を空にし、追加の入力を出す（ユーザ・機能は選択可）。 |
| 行を選ぶ | 一覧の行 | その割当を編集対象にする。ユーザ・機能・表示順を入れ、ユーザと機能は変更できなくする。 |
| 保存 | 主ボタン「保存」 | 必須が空なら送信せず、対象欄を示す。新規追加時はユーザ・機能・表示順を送る。編集対象選択時は表示順だけを送る。成功なら一覧を更新し、入力を閉じる。失敗ならエラーを示し、入力は残す。 |
| キャンセル | 副ボタン「キャンセル」 | 入力を捨て、選択を解除する。 |
| 解除 | テキストボタン「解除」 | 確認ダイアログで「解除」なら外す。「キャンセル」なら何もしない。成功なら一覧から消え、選択していれば選択を解除する。 |
...
| REQ-013 | 画面対象外（ログ） |
| REQ-014 | SCR-003 の行選択と保存（表示順の更新） |
...
| 2026-09-18 | 未承認 | SCR-002機能のアイコンを任意項目にする。「アイコンを削除する」チェックボックスを追加し、更新時にアイコンを外せるようにする |
| 2026-09-18 | 承認済み | SCR-002のアイコン任意項目化を承認 |
| 2026-09-19 23:43 | 未承認 | SCR-003に行選択→編集の操作を追加し、表示順を更新できるようにする（REQ-014） |
| 2026-09-19 23:47 | 承認済み | SCR-003の行選択→編集追加を承認 |
| 2026-09-22 13:18 | 未承認 | SCR-001のメールアドレス入力を必須から任意に変更。指定時のみ形式検査（REQ-003・004） |
| 2026-09-22 13:19 | 承認済み | メールアドレス任意化を承認 |
